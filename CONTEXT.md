...
- **JS engine** — the JavaScript runtime the browser needs for dynamic pages. wezig reaches it through a **`ScriptEngine` seam** (the same reversibility pattern as the `Renderer` seam, ADR-0005): a WIDE, DOM-coupled interface a BOUND engine (SpiderMonkey / JavaScriptCore / V8 — lean SpiderMonkey for independence) satisfies first for compatibility, with a Zig-native engine (e.g. `kiesel`) as an aspirational later swap-in behind the same seam. Pinned by `explore-native-renderer` (decision 3).
