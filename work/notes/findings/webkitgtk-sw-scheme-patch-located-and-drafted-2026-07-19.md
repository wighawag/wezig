...
- **Minimal patch captured as a `.patch`** in the task sidecar
  (`spike-webkitgtk-sw-scheme-patch-locate-and-draft/webkitgtk-sw-scheme-capable.patch`),
...
- **Size + footprint measured**
  (`spike-webkitgtk-sw-scheme-patch-locate-and-draft/patch-size-and-footprint.md`):
...
- **Upstream proposal DRAFTED (not filed)**
  (`spike-webkitgtk-sw-scheme-patch-locate-and-draft/upstream-proposal-draft.md`):
