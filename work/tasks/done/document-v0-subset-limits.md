...
Gate-2 BLOCK (ratified, must fix): the doc omits the v0 limit that block-box background-color and border colours are NOT cascaded end-to-end. See work/notes/observations/box-colours-not-cascaded-end-to-end-2026-07-14.md (ratified ACCEPT: the doc MUST state this explicitly). In src/paint.zig, paintBox leaves block/inline/anonymous boxes transparent and bg/border colours are caller-supplied above the seam via PaintStyle/GoldenScene; a real <div style=background-color:red> does NOT paint red in v0. FIX: in the CSS-properties section, mark background-color (and color for non-text/box fill) with a caveat, and in the layout/limits section add an explicit limit: 'box background-color and border colours are not yet resolved from the cascade end-to-end (v0 limit); only TEXT is painted from computed styles.' Keep everything else in the doc as-is; this is an additive accuracy fix so criterion 5 (matches reality) holds.
