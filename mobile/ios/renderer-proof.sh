#!/usr/bin/env bash
# Build + run the wezig iOS Renderer-backend proof on an iOS 17 Simulator (task
# ios-renderer-backend-oneshot, spec explore-mobile-shell story 4). The iOS twin
# of the desktop `zig build shell-test`: drive ONE real page THROUGH the pinned
# `Renderer` seam and assert navigate + a `.finished` lifecycle event reaching a
# seam subscriber + a non-blank `WKWebView.takeSnapshot`.
#
...
BUILD_DIR="${BUILD_DIR:-$IOS_DIR/build-renderer-proof}"
APP="$BUILD_DIR/WezigRendererProof.app"
BUNDLE_ID="dev.wighawag.wezig.rendererproof"
...
xcrun --sdk iphonesimulator swiftc \
    ...
    -o "$APP/WezigRendererProof" \
    "$IOS_DIR/Sources/RendererProof.swift"

# 3. Assemble the .app bundle (executable + Info.plist).
echo "== 3/5 assemble .app =="
sed 's/WezigShell/WezigRendererProof/g; s/dev\.wighawag\.wezig\.shell/dev.wighawag.wezig.rendererproof/g' \
...
UDID="$(xcrun simctl create wezig-ios17-renderer "$DEVTYPE" "$RUNTIME")"
echo "device: $UDID"
...
sleep 12
xcrun simctl spawn "$UDID" log show --last 40s --predicate 'eventMessage CONTAINS "iOS Renderer"' --style compact > "$BUILD_DIR/wezig.log" 2>&1 || true
kill "$LAUNCH_PID" 2>/dev/null || true

echo "---- launch.log ----"; cat "$BUILD_DIR/launch.log" || true
echo "---- wezig.log ----"; cat "$BUILD_DIR/wezig.log" || true

if grep -q "PASS: iOS Renderer seam drove one page" "$BUILD_DIR/launch.log" "$BUILD_DIR/wezig.log" 2>/dev/null; then
  echo "PASS: iOS Renderer backend drove one page through the pinned seam (finished + non-blank snapshot)."
else
  echo "FAIL: did not observe the iOS Renderer seam PASS line from the launched app."
  xcrun simctl shutdown "$UDID" || true
  exit 1
fi

xcrun simctl shutdown "$UDID" || true
echo "iOS Renderer-backend proof GREEN."
