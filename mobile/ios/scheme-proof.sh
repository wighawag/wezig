#!/usr/bin/env bash
# Build + run the wezig iOS custom-SCHEME proof on an iOS 17 Simulator
# (task mobile-web3-hooks-parity, spec explore-mobile-shell story 9). The iOS
# twin of the desktop `zig build shell-scheme-test`: serve ONE `wezig-test://hello`
# request from native THROUGH the pinned `Renderer` seam (WKURLSchemeHandler,
# registered on the config BEFORE the webview is created), mirroring the desktop
# proof.
#
...
# backend + its scheme-proof C-ABI) against the iOS-simulator SDK sysroot; compile
...
BUILD_DIR="${BUILD_DIR:-$IOS_DIR/build-scheme-proof}"
APP="$BUILD_DIR/WezigSchemeProof.app"
BUNDLE_ID="dev.wighawag.wezig.schemeproof"
...
xcrun --sdk iphonesimulator swiftc \
    ...
    -o "$APP/WezigSchemeProof" \
    "$IOS_DIR/Sources/SchemeProof.swift"

# 3. Assemble the .app bundle (executable + Info.plist).
echo "== 3/5 assemble .app =="
sed 's/WezigShell/WezigSchemeProof/g; s/dev\.wighawag\.wezig\.shell/dev.wighawag.wezig.schemeproof/g' \
...
UDID="$(xcrun simctl create wezig-ios17-scheme "$DEVTYPE" "$RUNTIME")"
echo "device: $UDID"
...
sleep 12
xcrun simctl spawn "$UDID" log show --last 40s --predicate 'eventMessage CONTAINS "iOS custom scheme"' --style compact > "$BUILD_DIR/wezig.log" 2>&1 || true
kill "$LAUNCH_PID" 2>/dev/null || true

echo "---- launch.log ----"; cat "$BUILD_DIR/launch.log" || true
echo "---- wezig.log ----"; cat "$BUILD_DIR/wezig.log" || true

if grep -q "PASS: iOS custom scheme served a native body that rendered" "$BUILD_DIR/launch.log" "$BUILD_DIR/wezig.log" 2>/dev/null; then
  echo "PASS: iOS custom scheme served a native body that rendered through the pinned seam."
else
  echo "FAIL: did not observe the iOS scheme PASS line from the launched app."
  xcrun simctl shutdown "$UDID" || true
  exit 1
fi

xcrun simctl shutdown "$UDID" || true
echo "iOS scheme-hook proof GREEN."
