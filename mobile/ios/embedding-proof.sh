#!/usr/bin/env bash
# Build + run the wezig iOS ViewHandle-embedding proof on an iOS 17 Simulator (task
# mobile-viewhandle-embedding-proof, spec explore-mobile-shell Q3/story 6). The iOS twin
# of the desktop `zig build shell-test` embedding: a mobile chrome-surface
# `embedView`s the renderer's WKWebView view (carried across the seam as the
# OPAQUE ViewHandle) and the page shows. Asserts a `.finished` lifecycle event
# reaching a seam subscriber + a non-blank snapshot of the EMBEDDED container
# (proving the opaque handle carried the view across a NON-GTK toolkit host,
# ADR-0007's flagged cross-toolkit-embedding spike, resolved on iOS).
#
...
# backend + its proof C-ABI) against the iOS-simulator SDK sysroot; compile the
# Swift proof app against the simulator SDK, linking the lib; assemble a minimal
# .app; boot an iOS 17 simulator; install; launch; assert the seam PASS line
# reached the device log.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
IOS_DIR="$REPO_ROOT/mobile/ios"
BUILD_DIR="${BUILD_DIR:-$IOS_DIR/build-embedding-proof}"
APP="$BUILD_DIR/WezigEmbeddingProof.app"
BUNDLE_ID="dev.wighawag.wezig.embeddingproof"
...
echo "== iOS ViewHandle-embedding-proof build =="
echo "SDK:            $SDK_PATH"
...
xcrun --sdk iphonesimulator swiftc \
    ...
    -o "$APP/WezigEmbeddingProof" \
    "$IOS_DIR/Sources/EmbeddingProof.swift"

# 3. Assemble the .app bundle (executable + Info.plist).
echo "== 3/5 assemble .app =="
sed 's/WezigShell/WezigEmbeddingProof/g; s/dev\.wighawag\.wezig\.shell/dev.wighawag.wezig.embeddingproof/g' \
...
UDID="$(xcrun simctl create wezig-ios17-embedding "$DEVTYPE" "$RUNTIME")"
echo "device: $UDID"
...
sleep 12
xcrun simctl spawn "$UDID" log show --last 40s --predicate 'eventMessage CONTAINS "iOS ChromeSurface"' --style compact > "$BUILD_DIR/wezig.log" 2>&1 || true
kill "$LAUNCH_PID" 2>/dev/null || true

echo "---- launch.log ----"; cat "$BUILD_DIR/launch.log" || true
echo "---- wezig.log ----"; cat "$BUILD_DIR/wezig.log" || true

if grep -q "PASS: iOS ChromeSurface embedded the renderer view" "$BUILD_DIR/launch.log" "$BUILD_DIR/wezig.log" 2>/dev/null; then
  echo "PASS: iOS ChromeSurface embedded the renderer's WKWebView via the opaque ViewHandle (page shown)."
else
  echo "FAIL: did not observe the iOS embedding-proof PASS line from the launched app."
  xcrun simctl shutdown "$UDID" || true
  exit 1
fi

xcrun simctl shutdown "$UDID" || true
echo "iOS ViewHandle-embedding proof GREEN."
